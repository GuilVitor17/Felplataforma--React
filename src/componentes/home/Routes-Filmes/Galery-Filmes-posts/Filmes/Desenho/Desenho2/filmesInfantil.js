...
const filmesInfantilInfomacoes = ({titulo, img,Id ,descricaoMin, data}) => {
 ...
 axios.delete(`${process.env.REACT_APP_BASE_URL_GALERY}/filmedesenho2/${Id}`, {
...
.then(async response => {
  let timerInterval
      ...
      await Swal.fire({
          position: 'top-end',
          icon: 'success',
          title: `FILME INFANTIL 2 EXCLUIDO`,
    ...
    <div className="Infantil">
              ...
              <div className="card">
                ...
                <h6 className="card-text">{descricaoMin}</h6>
                  ...
                  <Link className="Btn-livro" to={`/filmes/desenhos2/${Id}`}><button type="button" onClick={saveId} className="btn btn-primary">
                    Ver livro
                  </button></Link>
                  <div className="button-crud">
                  <Link className="button-editar" to={`/filmess/infantiledita2/${Id}`}><button>Editar</button></Link>
