...
const filmesRomanceInfomacoes = ({titulo, img,Id ,descricaoMin, data}) => {
 ...
 axios.delete(`${process.env.REACT_APP_BASE_URL_GALERY}/filmeromance5/${Id}`, {
...
.then(async response => {
  let timerInterval
      ...
      await Swal.fire({
          position: 'top-end',
          icon: 'success',
          title: `FILME ROMANCE 5 EXCLUIDO`,
    ...
    <div className="Romance">
              ...
              <div className="card">
                ...
                <h6 className="card-text">{descricaoMin}</h6>
                  ...
                  <Link className="Btn-livro" to={`/filmes/romances5/${Id}`}><button type="button" onClick={saveId} className="btn btn-primary">
                    Ver livro
                  </button></Link>
                  <div className="button-crud">
                  <Link className="button-editar" to={`/filmess/romanceedita5/${Id}`}><button>Editar</button></Link>
