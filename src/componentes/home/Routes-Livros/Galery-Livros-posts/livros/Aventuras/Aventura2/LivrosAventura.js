...
const LivrosAventuraInfomacoes = ({titulo, img,Id ,descricaoMin, data}) => {
 ...
 axios.delete(`${process.env.REACT_APP_BASE_URL_GALERY}/aventura2/${Id}`, {
  
})
.then(async response => {
  let timerInterval
          await Swal.fire({
            title: 'EXCLUINDO LIVRO 2 !',
            html: 'AGUARDE <b></b>',
            timer: 2000,
      ...
      await Swal.fire({
          position: 'top-end',
          icon: 'success',
          title: `LIVRO AVENTURA 2 EXCLUIDO`,
    ...
    <div className="Aventura">
              ...
              <div className="card">
                ...
                <h6 className="card-text">{descricaoMin}</h6>
                  ...
                  <Link className="Btn-livro" to={`/livros/aventuras2/${Id}`}><button type="button" onClick={saveId} className="btn btn-primary">
                    Ver livro
                  </button></Link>
                  <div className="button-crud">
                  <Link className="button-editar" to={`/livros/aventuraedita2/${Id}`}><button>Editar</button></Link>
